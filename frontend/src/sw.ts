...
const updateTaskStatusRoute = new Route(
  ({ request }) => {
    return request.url.includes(import.meta.env.VITE_API_BASE_URL + "/task/update-status");
  },
  new NetworkOnly({
    plugins: [bgSyncPlugin],
  }),
...
registerRoute(updateTaskStatusRoute);

const deleteTaskRoute = new Route(
  ({ request }) => {
    return request.url.includes(import.meta.env.VITE_API_BASE_URL + "/task/delete");
  },
  new NetworkOnly({
    plugins: [bgSyncPlugin],
  }),
